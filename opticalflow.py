...
DPI=1600; mm_dot=25.4/DPI;
scaling=200/24#(630.0/173.0) # calculated through testings(scaling= actual/returned)
        ...
        while (not self.terminationRequired):
            try:
                if self.has_been_called==False:
                    dis = getMouseEvent();
                    self.point_x = self.point_x + (scaling * dis.x);
                    self.point_y = self.point_y + (scaling * dis.y);
